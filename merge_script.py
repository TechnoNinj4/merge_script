...
#
    for index, row in merge_data.iterrows():
        if str(object = row[0]) in nontemplates:
            ...
            print("Document code "+row[0]+" was called for data ID number "+row[1]+".")
            if os.path.exists(templatepath+row[0]+'.docx'):
                print("A document template was detected for the document code called, proceeding to generate Microsoft Word document, please wait,...")
...
#
                ...
                filename = outputdir1+row[1]+'_'+row[0]+'_'+rt+'_'+usr+outputtype1
                document = MailMerge(templatefile)
                document.merge_pages([mrg_dt])
                document.write(filename)
                print(filename)
                if os.path.exists(filename):
                    print("Document generation process sucessful, check the MERGE_REVIEW folder for the new file.")
#
...
#
                ...
                print("Document code "+row[0]+" does not have an associated document template, the merge task for data ID number "+row[1]+" with said code was skipped.")
